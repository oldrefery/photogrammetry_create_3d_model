        ...
        with open(dense_mvs, 'rb') as f:
            ...
            while f.tell() < file_size:
                ...
                if chunk_type == 1:  # тип 1 - это точки
                    ...
                    print(f"Filtered out {len(points) - len(filtered_points)} distant points")
                    break
                else:
...
def create_3d_model(image_dir, output_dir, clean=False):
    ...
    try:
        # Preprocess images
        # preprocess_images(image_dir, target_size=800)
        print(f"Available memory: {psutil.virtual_memory().available / (1024 * 1024)} MB")
        print(f"Available CPUs: {psutil.cpu_count(logical=False)} physical cores")
    ...
    for img_path in image_files:
        try:
            ...
            if cv2.imwrite(img_path, img):
                print(f"Processed {os.path.basename(img_path)}: {width}x{height} -> {new_width}x{new_height}")
            else:
                print(f"Failed to save {os.path.basename(img_path)}")
        except Exception as e:
